...
function Show-ContextMenu {
    ...
    $listBox.Add_MouseClick({
        ...
        if ($e.Button -eq [System.Windows.Forms.MouseButtons]::Right) {
            ...
            if ($item) {
                Show-ContextMenu -Path $item.Tag -Control $listBox -MouseEvent $e
            }
        }
    })
            ...
            if ($node) {
                ...
                $newFolderMenuItem.Add_Click({
                    ...
                    if ($newFolderName -and $newFolderName -ne "") {
                        ...
                        if ($selectedNode) {
                            $newFolderPath = Join-Path $selectedNode.FullPath $newFolderName
                            New-Item -ItemType Directory -Path $newFolderPath
                            # Refresh the TreeView - you'll need to implement this function
                            Refresh-TreeView
                        }
                    }
                })
